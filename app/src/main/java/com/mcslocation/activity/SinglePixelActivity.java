...
public class SinglePixelActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if(Contants.DEBUG)
            Log.d(TAG,"MCS-->SinglePixelActivity-->onCreate--->启动1像素保活");
    ...
    protected void onDestroy() {
        if(Contants.DEBUG)
            Log.d(TAG,"MCS-->SinglePixelActivity-->onDestroy--->1像素保活被终止");
        if(! SystemUtils.isAPPALive(this,Contants.PACKAGE_NAME)){
            ...
            Log.i(TAG,"MCS-->SinglePixelActivity-->SinglePixelActivity---->APP被干掉了，我要重启它");
        }
        super.onDestroy();
    }
}
